...
CREATE OR REPLACE FUNCTION pgtest.f_create_error_message(s_returned_sqlstate TEXT, s_message_text TEXT, s_pg_exception_context TEXT)
  RETURNS varchar AS
$$
  SELECT coalesce(s_returned_sqlstate || ': ', '') ||
         coalesce(s_message_text, '') || E'\n' ||
...
BEGIN
  ...
  RAISE INFO 'Running tests in schema: %', s_schema_name;
  ...
  LOOP
    ...
    RAISE INFO 'Running test: %.%', s_schema_name, s_function_name;
    s_test_result := pgtest.f_run_test(s_schema_name, s_function_name);
    RAISE INFO '%', s_test_result;
  ...
  RAISE INFO 'Executed % tests of which % failed in %', i_test_count, i_error_count, (t_end_time - t_start_time);
